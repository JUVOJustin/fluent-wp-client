...
- Failed REST responses throw `WordPressApiError` (`kind: 'WP_API_ERROR'`)
- Failed local schema validation throws `WordPressSchemaValidationError` (`kind: 'SCHEMA_VALIDATION_ERROR'`)
- Local request/config failures throw `WordPressClientError` with kinds like `AUTH_ERROR`, `CONFIG_ERROR`, `NETWORK_ERROR`, and `PARSE_ERROR`
- Prefer `kind` for primary branching and use subclasses when you need subclass-only fields.
- See [Error handling](./errors.mdx) for the full guidance.

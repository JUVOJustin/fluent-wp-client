...
import { encodeZip as nt, collectFile as pt, StreamedFile as lt } from "@php-wasm/stream-compression";
import { logger as ae } from "@php-wasm/logger";
import { defineWpConfigConstants as ft, ensureWpConfig as ut } from "@wp-playground/wordpress";
import { unzipFile as jr, RecommendedPHPVersion as dt } from "@wp-playground/common";
function qr(r) {
...
class or {
  static async create(t) {
    const n = await ct(t), f = qr(t) ? t : void 0;
    return or.createFromDeclaration(n, f);
  }
...
const mt = `Blueprint resource of type "bundled" requires a filesystem.
...
function gt(r) {
  return r && typeof r == "object" && typeof r.resource == "string" && ht.includes(r.resource);
}
function bt(r) {
  try {
...
class qt extends hr {
  ...
  /** @inheritDoc */
  getURL() {
    return `https://downloads.wordpress.org/plugin/${Rr(this.resource.slug)}`;
  }
}
function Rr(r) {
...
}, xt = async (r, { options: t }) => await r.run(t), cr = async (r, { path: t }) => {
...
class WP_MySQL_Naive_Query_Stream {
  ...
  return n;
}, gr = async (r, { consts: t, method: n = "define-before-run" }) => {
  switch (n) {
...
}, xr = async (r, { command: t, wpCliPath: n = br }) => {
  await Nr(r, n);
  let f;
...
}, Ht = async (r, { fromPath: t, toPath: n }) => {
  (!t.startsWith("/") || !n.startsWith("/")) && ae.error(
    `
...
}, Zt = async (r, { writeToPath: t, filesTree: n }) => {
  t.startsWith("/") || (ae.error(
    `
...
async function Jt(r, t, n) {
  var f;
  (f = n == null ? void 0 : n.tracker) == null || f.setCaption("Importing content"), await wr(r, {
    path: "/tmp/import.wxr",
    data: t
  }), await r.run({
    $_SERVER: {
...
}, vr = async (r, { zipFile: t, zipPath: n, extractToPath: f }) => {
  if (n)
    ae.warn(
...
const ts = async (r, { pluginData: t, pluginZipFile: n, ifAlreadyInstalled: f, options: d = {} }, s) => {
  ...
  return r.unlink(n), r.unlink(f), _;
}, ps = `<?php
        ...
        if (E = E || p, !E) {
          ...
          if (!(_ && typeof _ == "object" && !Array.isArray(_)) && typeof _ != "string") {
            const K = {
              ...
              keyword: "type",
              params: {
                type: gs.additionalProperties.anyOf[1].type
              },
...
export {
  ...
  ts as installPlugin,
  ss as installTheme,
  Br as isBlueprintBundle,
  qs as isStepDefinition,
  is as login,
  Qt as mkdir,
  Ht as mv,
  Ut as request,
  ...
  vr as unzip,
  Wt as updateUserMeta,
  $s as validateBlueprint,
  xr as wpCLI,
  Ur as wpContentFilesExcludedFromExport,
  wr as writeFile,
  Zt as writeFiles,
  ns as zipWpContent
};

...
describe("Client: Comments", () => {
	...
	beforeAll(async () => {
		...
		if (!seedPost) {
			throw new Error(
				"Expected seeded post test-post-001 for comment integration coverage.",
			);
		}
	...
	describe("reads", () => {
		it("comments().list returns an array that includes the seeded comment", async () => {
			...
			expect(comments.some((comment) => comment.id === seedCommentId)).toBe(
				true,
			);
		});
		...
		it("comments().item fetches the seeded comment by ID", async () => {
			...
			expect(comment.content.rendered).toContain(
				"Client Comments: seeded comment",
			);
		});
		...
		it("comments().listPaginated returns pagination metadata", async () => {
			const result = await publicClient
				.comments()
				.listPaginated({ perPage: 1, page: 1, post: seedPostId });
	...
	describe("crud", () => {
		it("creates, updates, and deletes comments", async () => {
			...
			const deleted = await authClient
				.comments()

...
describe("Client: Posts", () => {
		...
		return new WordPressClient({
			...
			authHeaders: ({ method, url }) => {
				...
				if (!url.pathname.endsWith("/wp-json/wp/v2/posts")) {
					throw new Error(
						"Expected posts endpoint for request-aware post auth test.",
					);
				}
	...
	describe("reads", () => {
		...
		it("content('posts').item() returns undefined for non-existent slug", async () => {
			const post = await postsClient(publicClient).item(
				"this-slug-does-not-exist-999",
			);
		...
		it("content('posts').list() respects ordering", async () => {
			const asc = await postsClient(publicClient).list({
				orderby: "title",
				order: "asc",
			});
			const desc = await postsClient(publicClient).list({
				orderby: "title",
				order: "desc",
			});
	...
	describe("crud", () => {
		it("creates, updates, and deletes posts", async () => {
			...
			const updated = await postsClient(authClient).update(created.id, {
		...
		it("creates a post with content and excerpt", async () => {
			...
			expect(created.status).toBe("draft");
			expect(created.content.rendered).toContain(
				"Hello from client integration test.",
			);
